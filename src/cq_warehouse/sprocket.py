...
class Sprocket:
    ...
    Usage:
        ...
        s.object.rotate((0,0,0),(0,0,1),chain.spkt_initial_rotation[i])  # align to chain
    ...
    def __init__(self,
        ...
        bolt_circle_diameter : float
            the diameter of the mounting bolt hole pattern (default 0)
        num_mount_bolts : int
            the number of bolt holes (default 0) - if 0, no bolt holes are added to the
            sprocket
        mount_bolt_diameter : float
            the size of the bolt holes use to mount the sprocket (default 0)
        bore_diameter : float
            the size of the central hole in the sprocket (default 0) - if 0, no bore hole is
            added to the sprocket
        """
        self.num_teeth = num_teeth
        if not isinstance(self.num_teeth,int) or self.num_teeth<3:
            raise ValueError("num_teeth must be an integer >= 3")
        ...
        self.pitch_radius = Sprocket.sprocket_pitch_radius(self.num_teeth,self.chain_pitch)
        self.pitch_circumference = Sprocket.sprocket_circumference(self.num_teeth,self.chain_pitch)

        # Create the sprocket by extruding the tooth outline repeated around the sprocket
        self.cq_object = (cq.Workplane("XY")
            .polarArray(self.pitch_radius,0,360,num_teeth)
            .tooth_outline(self.num_teeth,self.chain_pitch,self.roller_diameter,self.clearance)
            .consolidateWires()
            .rotate((0,0,0),(0,0,1),90) # Align for sprocket rotation calculations
        ...
        _arc_list = { round(a.radius(),7) for a in self.cq_object.edges("%circle").vals() }
        if len(_arc_list)==3:     # three arcs imply "flat" teeth so chamfer
            self.cq_object = (
                self.cq_object.edges(cq.selectors.RadiusNthSelector(2))
                .chamfer(self.thickness*0.25,self.thickness*0.5)
            )
            self.outer_radius = self.pitch_radius+self.roller_diameter/4
        else:
            self.outer_radius = sqrt(
                self.pitch_radius**2-(self.chain_pitch/2)**2)   \
				+ sqrt((self.chain_pitch-self.roller_diameter/2)**2-(self.chain_pitch/2)**2
            )
        #
        # Create bolt holes
        if (self.bolt_circle_diameter!=0 and self.num_mount_bolts!=0 and
					self.mount_bolt_diameter!=0):
            self.cq_object = (
                self.cq_object.faces(">Z").workplane()
        ...
        if self.bore_diameter!=0:
            self.cq_object = self.cq_object.circle(self.bore_diameter/2).cutThruAll()
    ...
    def sprocket_circumference(num_teeth:int, chain_pitch:float) -> float:
        ...
        chain_pitch : float
            the distance between two adjacent pins in a single link (default 1/2 INCH)
        """
        return 2*pi*sqrt(chain_pitch*chain_pitch/(2*(1-cos(2*pi/num_teeth))))
...
def make_tooth_outline(
    ...
    outer_intersect_a_r=asin((outer_rad**3*(-(pitch_rad*sin(half_tooth_a))) + \
        sqrt(outer_rad**6*(-(pitch_rad*cos(half_tooth_a))**2) + \
        2*outer_rad**4*(chain_pitch-roller_rad)**2*(pitch_rad*cos(half_tooth_a))**2 + \
        2*outer_rad**4*(pitch_rad*cos(half_tooth_a))**4 + \
        2*outer_rad**4*(pitch_rad*cos(half_tooth_a))**2*(pitch_rad*sin(half_tooth_a))**2 -\
        outer_rad**2*(chain_pitch-roller_rad)**4*(pitch_rad*cos(half_tooth_a))**2 +\
        2*outer_rad**2*(chain_pitch-roller_rad)**2*(pitch_rad*cos(half_tooth_a))**4 +\
        2*outer_rad**2*(chain_pitch-roller_rad)**2*(pitch_rad*cos(half_tooth_a))**2*\
        (pitch_rad*sin(half_tooth_a))**2-outer_rad**2*(pitch_rad*cos(half_tooth_a))**6 -\
        2*outer_rad**2*(pitch_rad*cos(half_tooth_a))**4*(pitch_rad*sin(half_tooth_a))**2 -\
        outer_rad**2*(pitch_rad*cos(half_tooth_a))**2*(pitch_rad*sin(half_tooth_a))**4) +\
        outer_rad*(chain_pitch-roller_rad)**2*(pitch_rad*sin(half_tooth_a)) -\
        outer_rad*(pitch_rad*cos(half_tooth_a))**2*(pitch_rad*sin(half_tooth_a)) -\
        outer_rad*(pitch_rad*sin(half_tooth_a))**3)/(2*(outer_rad**2*\
        (pitch_rad*cos(half_tooth_a))**2+outer_rad**2*(pitch_rad*sin(half_tooth_a))**2)))
    ...
    tangent_pt = cq.Vector(0,-roller_rad).rotateZ(-tooth_a_degrees/2) + \
                 cq.Vector(pitch_rad,0).rotateZ(tooth_a_degrees/2)
    # The intersection point of the tooth and the outer rad
    outer_pt = cq.Vector(outer_rad*cos(outer_intersect_a_r),outer_rad*sin(outer_intersect_a_r))
    # The location of the tip of the spike if there is no "flat" section
    spike_pt = cq.Vector(
        sqrt(pitch_rad**2-(chain_pitch/2)**2)+sqrt((chain_pitch-roller_rad)**2-(chain_pitch/2)**2),
        0
    )
...
def _tooth_outline(self,num_teeth,chain_pitch,roller_diameter,clearance) -> cq.Workplane:
