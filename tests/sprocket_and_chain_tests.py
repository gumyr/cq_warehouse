...
class TestParsing(BaseTest):
    ...
    def test_sprocket_input_parsing(self):
        ...
        with self.assertRaises(ValueError):     # Insufficient tooth count
            Sprocket(2)
        with self.assertRaises(ValueError):     # Not int
            Sprocket(10.5)
...
class TestSprocketShape(BaseTest):
    ...
    def test_flat_sprocket_shape(self):
        ...
        # cq.exporters.export(s,"sprocket_flat.step")
    ...
    def test_spiky_sprocket_shape(self):
        ...
        spkt = Sprocket(num_teeth=16,chain_pitch=0.5*INCH,roller_diameter=0.49*INCH).cq_object.val()
        # cq.exporters.export(s,"sprocket_spiky.step")
        self.assertTrue(spkt.isValid())
...
class TestChainShape(BaseTest):
    ...
    def test_five_sprocket_chain(self):
        ...
        roller_pos=[(10.18715872390352, 222.87862587734543, 0.0),
    ...
    def test_missing_link(self):
        ...
        with self.assertWarns(Warning):
            Chain(spkt_locations = [cq.Vector(-4.9*INCH,0,0),cq.Vector(+5*INCH,0,0)])

class TestSprocketMethods(BaseTest):
    """ Sprocket class methods """
    def test_sprocket_pitch_radius(self):
        """ Pitch radius verification """
        self.assertAlmostEqual(Sprocket.sprocket_pitch_radius(32,0.5*INCH),64.78458745735234)
    def test_sprocket_circumference(self):
        """ Pitch circumference verification """
        self.assertAlmostEqual(Sprocket.sprocket_circumference(32,0.5*INCH),407.0535680437272)
...
class TestChainMethods(BaseTest):
    ...
    def test_gen_mix_sum_list(self):
        ...
        self.assertEqual(Chain._gen_mix_sum_list([1,2,3,4],[3,4,1,2]), [1, 4, 6, 10, 13, 14, 18, 20])
    def test_interleave_lists(self):
        """ Validate custom list function """
        with self.assertRaises(ValueError):
            Chain._interleave_lists([1,2],[1,2,3])
        self.assertEqual(Chain._interleave_lists([1,2,3,4],[3,4,1,2]), [1, 3, 2, 4, 3, 1, 4, 2])
    ...
    def test_assemble_chain_transmission(self):
        ...
        transmission = (
            chain.
            assemble_chain_transmission([spkt0.cq_object,spkt1.cq_object])
            .rotate((1,0,0),90).translate((0,0,-40))
        )
...
class TestVectorMethods(BaseTest):
    ...
    def test_vector_rotate(self):
        ...
        self.assertTupleAlmostEquals(vector_x.toTuple(), (1,-math.sqrt(2)/2,math.sqrt(2)/2), 7)
    ...
    def test_vector_flip_y(self):
        ...
        self.assertTupleAlmostEquals(cq.Vector(1,2,3).flipY().toTuple(), (1,-2,3), 7)
    def test_point_to_vector(self):
        ...
        self.assertTupleAlmostEquals(point.pointToVector('XY',4).toTuple(), (1,2,4), 7)
        self.assertTupleAlmostEquals(point.pointToVector('XZ',3).toTuple(), (1,3,2), 7)
        self.assertTupleAlmostEquals(point.pointToVector('YZ',5).toTuple(), (5,1,2), 7)
